<template>
  <div class="mx-4">
    ...
    <hr class="my-4">
    <div
      class="flex flex-col rounded-lg shadow divide-y-0 md:divide-y-2 text-gray-500"
    >
      <div
        v-for="task in tasks"
        :key="task.id"
        class="p-4 md:bordered relative"
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            <span
              :class="statusClass(task.status.value)"
              class="mr-2 px-2 rounded-full text-sm font-semibold"
            >
              {{ task.status.label }}
            </span>
        ...
        <div class="flex flex-col md:flex-row gap-x-2 gap-y-1 text-sm mt-2">
          ...
          <strong> Updated at {{ formatDateTime(task.updatedAt) }} </strong>
        </div>
      </div>
    </div>
  </div>
